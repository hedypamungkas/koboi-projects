...
- **D · insurance-claims `policy.rules` case-sensitive bypass** — `config/agent.yaml:127-134`. Glob `*total loss*` is case-sensitive; "Total Loss"/"TOTAL LOSS" bypasses the deny rule and gets an auto-settlement recommendation. *Fix:* case-insensitive match, or add upper/title variants, or enforce in-tool.
...
- Config/docker: 10/10 compose `healthcheck` + `depends_on: service_healthy`; 10/10 backend Dockerfiles non-root `USER app`; root `.dockerignore`; concierge auth-key default + front-door `sandbox:restricted`; legal `sandbox:restricted`; CS + insurance `rlimits`+`git_init`; `koboi-agent==0.18.2` pin (ecommerce/legal/healthcare); webhook URLs `host.docker.internal` (CS/insurance/market-intel); docs/00 "6 apps"→"10 apps"; insurance compose "(DAG)"→"(single-agent)". (`test_cross_cutting` parameterized over all 10.)
